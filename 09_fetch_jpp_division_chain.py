"""09_fetch_jpp_division_chain.py — JPP 分割出願情報タブから親系列を完全取得

ユーザー指示: API では8世代までしか追えない（最先記録なし）が、
              JPP の「分割出願情報」タブは第12世代まで取れる（本願の祖先系列が完全）。
              よって JPP を正の経路にし、API は使わない。

経路:
  1. JPP トップ → 番号照会 → 出願番号 (YYYY-NNNNNN)
  2. 経過情報ボタン → 新ウィンドウ
  3. 「分割出願情報」タブをクリック
  4. テーブル/ノードから世代別の出願番号と出願日を抽出

出力:
  inventory/jpp_division_chains/{appno}.json   案件別の分割系列
  inventory/jpp_division_log.tsv               実行ログ

CLI:
  python 09_fetch_jpp_division_chain.py --appno 2023026797
  python 09_fetch_jpp_division_chain.py             # 全44件
  python 09_fetch_jpp_division_chain.py --force     # 既存も再取得
"""
...
OUT_DIR = INV_DIR / "jpp_division_chains"
LOG_PATH = INV_DIR / "jpp_division_log.tsv"
