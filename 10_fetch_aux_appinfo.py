"""10_fetch_aux_appinfo.py — 補助ソースの出願情報タブから親出願番号を再帰取得

用途: 分割出願の系列追跡（API 不正確のため 補助ソース 一本化）

各号について 経過情報→「出願情報」タブを開き、以下を抽出:
  - 出願日
  - 親出願番号（あれば）
  - 「分割（44条1項）」フラグ
  - 国内優先有無
...
CLI:
  python 10_fetch_aux_appinfo.py --appno 2023026797
  python 10_fetch_aux_appinfo.py             # 全44件
"""
...
OUT_DIR = INV_DIR / "aux_appinfo"
LOG_PATH = INV_DIR / "aux_appinfo_log.tsv"
...
def parse_app_info(html: str) -> dict:
    ...
    補助ソース実物観察に基づく抽出（2024-006538 で確認）:
...
def build_aux_chain(start_appno: str, page: Page, context: BrowserContext,
...
def main() -> None:
    ...
    with sync_playwright() as pw:
        ...
        for i, (case_key, appno) in enumerate(targets, 1):
            ...
            try:
                chain = build_aux_chain(appno, page, context, fetch_calls)
