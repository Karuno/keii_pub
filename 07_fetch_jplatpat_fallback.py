"""07_fetch_jplatpat_fallback.py — J-PlatPat 経過情報全書類分の起案日／受領日を取得

JPO API 不通時の **代替経路（fallback）**。 1案件あたり以下を1セッションで取得:
...
出力:
  inventory/jplatpat_dates/{appno}.json   ← 統合書類リスト
  inventory/jplatpat_dates_log.tsv        ← 実行ログ

CLI:
  python 07_fetch_jplatpat_fallback.py                 # 全件
  python 07_fetch_jplatpat_fallback.py --limit 3       # パイロット
  python 07_fetch_jplatpat_fallback.py --appno 2018244177
  python 07_fetch_jplatpat_fallback.py --force         # 既存も再取得
"""
...
OUT_DIR = INV_DIR / "jplatpat_dates"
LOG_PATH = INV_DIR / "jplatpat_dates_log.tsv"
