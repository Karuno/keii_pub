...
Lievito ([lievito.work](https://lievito.work/)) で動いている **「手続の経緯」生成エンジン** のオープンソース実装。
ユーザーから寄せられたフィードバックは日次の進化ループ経由でこのリポジトリにコミットされ、本番に反映される。

審決起案の「手続の経緯」節を、Python ルールベース（LLM フリー）で自動生成する。
入力は **出願番号のみ**。分割出願（多世代）／優先権（パリ・国内）／PCT 国内段階／外国語書面出願 とそれらの複合に対応する。

**変更履歴**: [Commits on main](https://github.com/Karuno/keii_pub/commits/main) で全変更を追跡できる。
進化ループ由来のコミットは `evolve:` プレフィックスが付く。
...
- 標準ライブラリ以外の依存:
  ...
  - 任意: `playwright`（補助ソース取得スクリプトを使う場合のみ）
...
5. python3 10_fetch_aux_appinfo.py    ← aux_appinfo/ (補助ソース, playwright 必要)
