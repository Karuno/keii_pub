...
[Step 1] データ取得 ────────────────────┐
   ...
   ├─ zenchi_drafting/ (J-PlatPat)      │ inventory/
   ├─ jpp_app_info/ (J-PlatPat)         │ から参照する
   └─ jplatpat_dates/ (フォールバック)   │
                                          │
   ▼                                     ▼
...
| 項目 | 内容 |
|---|---|
| 情報源 | J-PlatPat（特許情報プラットフォーム）の経過情報ページ |
| 取得スクリプト | `06_fetch_zenchi_drafting.py` |
| ファイル形式 | JSON |

前置報告書（書類コード `A913`）の本文に記載される「**作成日 元号N年M月D日**」を取得。
JPO API は本書類の本文配信対象外のため J-PlatPat を使う。

### 3.4 `jpp_app_info/{appno}.json` — 分割チェーン

| 項目 | 内容 |
|---|---|
| 情報源 | J-PlatPat の「出願情報」ページ |
| 取得スクリプト | `10_fetch_jpp_app_info.py` |
| ファイル形式 | JSON |
...
### 3.5 `jplatpat_dates/{appno}.json` — フォールバック

JPO API 不通時のフォールバック。J-PlatPat 経過情報を全件スクレイピングして、
書類名と日付の一覧を取得する（精度は API より低い）。

| 取得スクリプト | `07_fetch_jplatpat_fallback.py` |
...
| 07 | `07_fetch_jplatpat_fallback.py` | フォールバック全書類日付 |
| 08 | `08_fetch_parent_chain.py` | 親出願チェーン |
| 09 | `09_fetch_jpp_division_chain.py` | 分割チェーン（旧経路） |
| 10 | `10_fetch_jpp_app_info.py` | 分割チェーン（現行経路） |
...
| 世代数 | `jpp_app_info/{appno}.json` の `chain` の長さ − 1 |
...
|---|---|---|
| **A**（庁作成）通常 | `doc_xmls/{appno}/_summary.json` → 各 XML の `<jp:drafting-date>` | `jplatpat_dates/{appno}.json` |
| **A** 前置報告書 (A913) | `zenchi_drafting/{appno}.json` の `drafting_dates_all` | （同上） |
| **B**（提出） | `doc_history_collected/{appno}.json` の各書類の `legalDate` | `jplatpat_dates/{appno}.json` の `table_date` |
...
5. python3 10_fetch_jpp_app_info.py    ← jpp_app_info/ 取得（J-PlatPat）
6. python3 06_fetch_zenchi_drafting.py ← zenchi_drafting/ 取得（J-PlatPat、前置報告書のみ）
7. python -m generator.keii {appno}    ← 生成
```

J-PlatPat 取得（06, 10）は API 不要だが、Playwright のセットアップが必要。
...
| J-PlatPat | 工業所有権情報・研修館の特許情報プラットフォーム |
