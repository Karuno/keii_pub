...
冒頭文は「本願の性質」によって 12 パターンに分かれる。`generator/classify.py` の
...
[Step A] 分割案件か？ (parentApplicationInformation あり、かつ出願日が本願と異なる)
   ...
   │                │
   │      ┌─────────┴────────┐
   │  親doc_historyのdivisionalApplicationInformation に本願 appno が無い
   │     → 「変更_実用新案_特許」（46条）★新設
   │  世代≥3 → 「分割_第3世代以降」
   │  世代=2 で 最先がPCT → 「分割_PCT原出願_第2世代」
   │  世代=2 → 「分割_第2世代」
...
| 変更_実用新案_特許 | 「本願は、{元実用新案登録出願日}の実用新案登録出願である{実願番号}を{本願出願日}に特許出願に変更した出願（特願{本願番号}号）であって、…」 | 46 条 1 項 |

**46条出願変更の検出**（`classify.py:is_utility_model_conversion()`）:

`parentApplicationInformation` がある分割系列のうち、親 doc_history の `divisionalApplicationInformation` に本願 appno が含まれていなければ 46 条による出願変更（実用新案登録出願 → 特許出願）と判定する。44 条 1 項の分割では親側にも子の登録が必須のため、親側に登録が無いことは 46 条の強いシグナルとなる。

- 親 doc_history が未取得（`inventory/doc_history_collected/{parent_appno}.json` 無し）の場合は判定不能 → False（44条扱いフォールバック）
- 親 doc_history を確実に確保するため `fetcher/onboard_appno.py:_fetch_parent_chain` を本願 onboard 時に併せて実行する運用
...
| 翻訳文 | 表記 |
|---|---|
| あり（A631 / A632 / A634、または documentDescription に「翻訳文」を含む） | 外国語書面出願 |
| なし | 特許出願 |

判定実装は `classify.py:has_translation_doc()` 経由。A631 の documentDescription が「翻訳文提出書」、A634 が「国際出願翻訳文提出書」なので、「翻訳文」キーワードでも拾える。
...
#### Type B
- `A53` 意見書
- `A521` / `A522` / `A523` 手続補正書（**「（方式）」付きは除外**、明細書/特許請求の範囲を補正対象に含まないものも除外）
- `A524` 誤訳訂正書
- `A631` 翻訳文の提出（翻訳文提出書、外国語書面出願系）
- `A632` 翻訳文の提出（国内書面、PCT国内移行系）
- `A634` 翻訳文の提出（国際出願翻訳文提出書、PCT国内移行系）
- `A781` 上申書（**前置報告書より前にあるものは除外**：開発者ルール docx コメント [7]）
- `A971015` 応対記録
- `C60` 審判請求書

翻訳文の採用日決定は `A634` > `A632` > `A631` の優先順 (`dates.py:_from_doc_history_json`)。

これ以外の書類は生成器が出力しない。詳細な書類コード突合は `docs/doc_code_mapping.md` を参照。
...
- 結合内順序（コーパス基準。実装: `chronology.py:_combine_same_day_b`）:
  - 誤訳訂正書 → 審判請求書 → 意見書 → 上申書 → 手続補正書 → 翻訳文の提出 → 国内書面の提出 → 応対記録/面接
- 連結語（コーパス基準。corpus 多数派 約 6:1 で「、」優位のため、公用文一律ルールから読みやすさ優先で外れる。実装: `chronology.py:_format_b_doc_names`）:
  - **2 要素**: 「Ａ、Ｂ」（「、」のみ）
  - **3 要素以上**: 「Ａ、Ｂ、Ｃ及びＤ」（最後だけ「及び」、他は「、」）
  - 例: 「意見書、手続補正書の提出」「審判請求書、意見書、手続補正書及び上申書の提出」
- 応対記録／面接は単体使用が多く「の提出」を付けない。
...
### 13.1 旧指標（ratio mean、2026-05-20 時点 corpus 44 件）
...
### 13.2 新指標（正規化後完全一致 Y/N の正答率、2026-06-07 導入）

旧 `ratio` (difflib.SequenceMatcher) は文字単位の甘い指標で、書類 1 件抜けでも 0.9 を超えてしまう問題があった。これを「正規化後の完全一致 (Y/N) で計測し、正答数 / 総件数 = 正答率」に切り替える。実装: `tools/keii_normalize.py` + `92_batch_generate.py` の `match` 列。
