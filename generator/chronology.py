...
def _render_type_c(date_text: str) -> str:
    ...
    括弧「（」を1文字使うぶん、外側の先頭全角空白は (10 - len(date_text)) 個にする。
    これにより括弧内の日付が他行の日付と同じ桁から始まり、「：」が位置14でそろう。
    例: date_text='同年　５月　９日'(8文字) → 先頭空白2個 → 「　　（同年　５月　９日　　：…」。
    """
    pad = 10 - len(date_text)
    if pad < 0:
        pad = 0
    return "　" * pad + f"（{date_text}　　：原査定の謄本の送達）"


def _format_b_doc_names(names: list[str]) -> str:
    """Type B 書類名を公用文ルールに従って結合し、末尾に「の提出」を付与。

corpus 多数派 (約 6:1 で「、」優位) に合わせる。
      公用文ルールでは 2 要素「Ａ及びＢ」が原則だが、書類名が連続すると読みづらく、
      審決起案実務では「Ａ、Ｂ」を選ぶ傾向が強いため:
      - 2要素: 「Ａ、Ｂ」 (公用文ルールから外し、読みやすさ優先)
...
def _pad_date_to_position11(date_text: str) -> str:
    """日付文字列を「位置11 (=日) で終わる」よう先頭に全角空白でパディング。

    月日は _pad_md で2桁幅に揃っている前提。各パターンの幅:
      「令和N年MM月DD日」  → 10〜11文字（1桁の元号年は10、2桁は11）
      「同年MM月DD日」     → 8文字  → 先頭3全角空白
      「同月DD日」         → 5文字  → 先頭6全角空白
      「同日」             → 2文字  → 先頭9全角空白

    フル元号も含めて11全角幅へ右寄せする。1桁元号年（「令和４年」等）や
    元年（「令和元年」）は10文字なので先頭に全角空白1個を補い、後続の
    「付け：」「　　：」が位置12〜14、すなわち「：」が常に位置14でそろう。
    """
    pad = 11 - len(date_text)
    if pad > 0:
        return "　" * pad + date_text
    return date_text
...
def render_chronology(entries: list[DocumentEntry]) -> list[ChronoLine]:
    ...
    for date_iso, doc_type, group in groups:
        ...
        if doc_type == "A":
            ...
            elif e.name == "当審拒絶理由通知書":
                # 経緯リスト上の表記は原審のものと同じ「拒絶理由通知書」
                # (「当審」の限定は地の文で導入する。確定審決コーパスの実例準拠)
                doc_name = "拒絶理由通知書"
            # 起案日取得不可の場合 (補助ソース 未整備時) は date を参照エラー表記に
            if (e.note or "") == "drafting_date_unavailable":
                line = (
                    "<<参照エラー: " + e.name + " の起案日が公開APIで取得不可"
                    " (発送日 " + date_text + " のみ取得済)>>"
                )
            else:
